...
function ar.load(path)
    ...
    file.read = function(c) if c then
        ...
        if file.seek then retval = oldread(c) else
            for i = 1, c do
                ...
                retval = (retval or "") .. n
                if (seek + i) % 102400 == 0 then os.queueEvent(os.pullEventRaw()) end
            end
        end
        seek = seek + c
        return retval
    else return oldread() end end
    if file.read(8) ~= "!<arch>\n" then
        file.close()
    ...
    if name_table then for k,v in pairs(name_rep) do
        local n = tonumber(string.match(v, "/(%d+)")) or 0
...
function ar.read(p)
    local file,e = fs.open(p, "r")
    local retval = {
        name = fs.getName(p),
        timestamp = os.epoch and math.floor(os.epoch("utc") / 1000) or 0, 
        owner = fs.getOwner and fs.getOwner(p) or 0, 
        group = 0,
        mode = fs.getPermissions and cc2u(fs.getPermissions(p, fs.getOwner(p) or 0)) * 0x40 + cc2u(fs.getPermissions(p, "G")) + bit.band(fs.getPermissions(p, "*"), 0x10) * 0x80 or 0x1FF,
        data = ""
    }
    if file.seek then retval.data = file.readAll() else
        local c = file.read()
        while c ~= nil do 
...
function ar.save(data, path)
    ...
    file.write = function(str) 
        for c in string.gmatch(str, ".") do oldwrite(c) end
